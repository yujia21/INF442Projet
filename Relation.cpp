...
Relation Relation::join (Relation *r, std::vector<std::string> list1, 
   ...
   if (size1 == 0 || size2 == 0){ //TO DO use assert or raise exceptions?
      ...
      vector<string> commonvar(list1.size()+list2.size());
      vector<string>::iterator it;
      
      it = set_intersection(list1.begin(), list1.end(), 
            list2.begin(), list2.end(), commonvar.begin());
      commonvar.resize(it-commonvar.begin());
      //std::back_inserter(common)
      //use vectors ? or set with keys as numbers?
      
      std::cout << (commonvar.size()) << " common elements:\n";
      for (it=commonvar.begin(); it!=commonvar.end(); ++it)
        std::cout << ' ' << *it;
      std::cout << '\n';      
